...
def regression(model, params):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if (epoch + 1) % 20 == 0:
                model_save_path = f"./models/resnet50_model_epoch{epoch+1}_fold{fold}.pth"
                torch.save(model.state_dict(), model_save_path)  # Save the model state_dict
                print(f"Model saved at {model_save_path}")
                mlflow.log_artifact(model_save_path)  # Optionally log to mlflow as an artifact
