...
def regression(model, params):
    ...
    for epoch in range(num_epochs):
        ...
        all_outputs = []
        all_labels = []
        running_drloc_loss = 0.0
        for images, labels in tqdm(train_loader):
            images, labels = images.to(device), labels.to(device).float()

            optimizer.zero_grad()
            outputs, drloc_loss = model(images, m)
            drloc_loss *= labmda_

            loss = loss_func(outputs, labels)
            loss += drloc_loss
            running_drloc_loss += drloc_loss
        ...
        train_drloc_loss = running_drloc_loss / len(train_loader)
        ...
        mlflow.log_metric('train_DRLoc_loss', train_drloc_loss, step=epoch+1) # mlflow에 train drloc loss 기록

        print(f"\nEpoch {epoch+1} Summary:")
        if fold:
            print(f"Fold: {fold}/{n_folds}")
        print(f"Train Loss: {running_loss / len(train_loader):.3f}")
        print(f"DRLoc Loss: {running_drloc_loss / len(train_loader):.3f}")
